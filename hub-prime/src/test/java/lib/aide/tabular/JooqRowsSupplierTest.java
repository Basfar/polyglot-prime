...
public class JooqRowsSupplierTest {
        ...
        SELECT 
        "country", 
        MAX(CASE WHEN "sport" = 'Athletics' THEN "gold" ELSE NULL END) AS "Athletics", 
        MAX(CASE WHEN "sport" = 'Swimming' THEN "gold" ELSE NULL END) AS "Swimming"
        FROM medals
        GROUP BY "country"
        OFFSET ? ROWS
        FETCH NEXT ? ROWS ONLY
            """;
                ...
                final var actualSQL = jooqQuery.query().getSQL();
                final var expectedParams = List.of(request.startRow(), request.endRow() - request.startRow());
                assertThat(actualSQL.toLowerCase())
                        .isEqualToIgnoringWhitespace(expectedSQL.toLowerCase());
                assertThat(jooqQuery.bindValues()).isEqualTo(expectedParams);
        }
